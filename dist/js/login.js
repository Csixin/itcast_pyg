...
$(function () {
  $("#pyg-login").on("tap", function () {
    var username_text = $("input[name='username']").val().trim();
    var password_text = $("input[name='password']").val().trim(); //验证账号和密码合法性
    ...
    $.ajax({
      ...
      success: function success(res) {
        if (res.meta.status == 200) {
          mui.toast("成功登陆");
          var pageUrl = sessionStorage.getItem("pageUrl"); //把token存进sessinStorage,顺手把res.data全塞进去
